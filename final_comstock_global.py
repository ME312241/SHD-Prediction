...
MODEL_SAVE_DIR = Path(r"A:\College\Thesis\Models\comstock_global_rev1")
...
def make_actual_vs_pred_plot(y_true, y_pred, title_name, out_path):
...
map_first_match(df, "out.params.hours_heating_setpoint_not_met",
                exact_names=[
...
engineered = [
    ...
    "operating_hours_week", "heating_setpoint_span_c",
...
print(f"Global model includes climate zone feature: {CLIMATE_COL in features}")
...
for n, m in models:
    ...
    stats[n] = {"rmse": rmse_te, "gap": gap, "gen_score": gen_score, "mae": mae, "r2": r2}
...
print(f"Baseline(global median) RMSE: {baseline_rmse:.3f}")
print(f"Selected global model by GenScore: {best_name}")

plot_path = PLOTS_DIR / f"actual_vs_pred_global_{best_name}.png"
make_actual_vs_pred_plot(y_test, best_pred, f"Global - {best_name}", plot_path)

# save artifacts
joblib.dump(xgb_model, MODEL_SAVE_DIR / "xgb_global.joblib")
joblib.dump(cat_model, MODEL_SAVE_DIR / "catboost_global.joblib")
joblib.dump(lgb_model, MODEL_SAVE_DIR / "lightgbm_global.joblib")
joblib.dump(stack_model, MODEL_SAVE_DIR / "stacking_global.joblib")
joblib.dump(encoders, MODEL_SAVE_DIR / "encoders_global.joblib")
joblib.dump(features, MODEL_SAVE_DIR / "features_global.joblib")
...
joblib.dump(
    ...
    MODEL_SAVE_DIR / "tuning_summary_global.joblib",
)
